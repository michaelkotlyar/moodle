...
final class override_test extends \advanced_testcase {
    ...
    /**
     * Test quiz override for quizzes without SEB configuration.
     *
    ...
    public function test_override_settings_changed(): void {
        ...
        // Check SEB settings are normal for both user.
        foreach ([$this->overrideuser, $this->user] as $user) {
            $this->setUser($user);
        ...
        $loadedquizconfigkey = seb_quiz_settings::get_config_key_by_quiz_id($this->quiz->id);
        ...
        // Test config.
        $userquizconfig = seb_quiz_settings::get_config_by_quiz_id($this->quiz->id); // Overriden config.

        $quizconfig = \cache::make('quizaccess_seb', 'config')->get($this->quiz->id);
        $this->assertNotEquals($quizconfig, $userquizconfig);

        $overridenquizconfig = \cache::make('quizaccess_seb', 'config')->get("{$this->quiz->id}-$overrideid");
        $this->assertEquals($overridenquizconfig, $userquizconfig);

        // Test config key.
        $userquizconfigkey = seb_quiz_settings::get_config_key_by_quiz_id($this->quiz->id); // Overriden key.

        $quizconfigkey = \cache::make('quizaccess_seb', 'configkey')->get($this->quiz->id);
        $this->assertNotEquals($quizconfigkey, $userquizconfigkey);

        $overridenquizconfigkey = \cache::make('quizaccess_seb', 'configkey')->get("{$this->quiz->id}-$overrideid");
        $this->assertEquals($overridenquizconfigkey, $userquizconfigkey);

        // Test normal user is not overriden.
        // Test settings.
        $this->setUser($this->user);
        $sebconfig = seb_quiz_settings::get_by_quiz_id($this->quiz->id);
        ...
        $quizconfig = \cache::make('quizaccess_seb', 'config')->get($this->quiz->id);
        $this->assertEquals($quizconfig, $userquizconfig);

        $overridenquizconfig = \cache::make('quizaccess_seb', 'config')->get("{$this->quiz->id}-$overrideid");
        $this->assertNotEquals($overridenquizconfig, $userquizconfig);

        // Test config key.
        $userquizconfigkey = seb_quiz_settings::get_config_key_by_quiz_id($this->quiz->id); // Overriden key.

        $quizconfigkey = \cache::make('quizaccess_seb', 'configkey')->get($this->quiz->id);
        $this->assertEquals($quizconfigkey, $userquizconfigkey);

        $overridenquizconfigkey = \cache::make('quizaccess_seb', 'configkey')->get("{$this->quiz->id}-$overrideid");
        $this->assertNotEquals($overridenquizconfigkey, $userquizconfigkey);
    }
