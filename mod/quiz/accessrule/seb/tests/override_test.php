...
final class override_test extends \advanced_testcase {
    ...
    public function test_override_settings(): void {
        ...
        $settings = $this->get_test_settings(['quizid' => $this->quiz->id]);
        ...
        // Check that settings are not equal.
        ...
        // Check that settings are now equal.
        $this->assertEquals($configs[0], $configs[1]);

        // Remove settings.
        $quizsettings->delete($this->quiz->id);

        // Check users no longer have SEB settings.
        foreach ($users as $user) {
            $this->setUser($user);
            $raised = false;
            try {
