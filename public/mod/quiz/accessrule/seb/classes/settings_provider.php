...
class settings_provider {
    ...
    public static function is_conflicting_permissions(context_course|context_module $context, bool $isoverride = false): bool {
        ...
        switch ($settings->get('requiresafeexambrowser')) {
            ...
            case self::USE_SEB_CONFIG_MANUALLY:
                return $isoverride
                    ? !has_capability('quizaccess/seb:manage_seb_configuremanually_override', $context)
    ...
    public static function get_requiresafeexambrowser_options(
        ...
        if (self::can_donotrequire($context) || self::is_conflicting_permissions($context, $isoverride)) {
            $options[self::USE_SEB_NO] = get_string('no');
        }

        if (self::can_configure_manually($context) || self::is_conflicting_permissions($context, $isoverride)) {
            $options[self::USE_SEB_CONFIG_MANUALLY] = get_string('seb_use_manually', 'quizaccess_seb');
        }

        if (self::can_use_seb_template($context) || self::is_conflicting_permissions($context, $isoverride)) {
        ...
        if (!$isoverride && (self::can_upload_seb_file($context) || self::is_conflicting_permissions($context))) {
            $options[self::USE_SEB_UPLOAD_CONFIG] = get_string('seb_use_upload', 'quizaccess_seb');
        }

        if (self::can_use_seb_client_config($context) || self::is_conflicting_permissions($context, $isoverride)) {
