...
class settings_provider {
     ...
     * @param bool $isoverride If true, check for override capabilities instead of normal capabilities.
     *
     * @return bool
     */
    public static function is_conflicting_permissions(context_course|context_module $context, bool $isoverride = false): bool {
        ...
        if ($isoverride) {
            if (
                self::can_use_seb_client_config($context, $isoverride)
                || self::can_upload_seb_file($context, $isoverride)
                || self::can_donotrequire($context, $isoverride)
                || self::can_configure_manually($context, $isoverride)
                || self::can_use_seb_template($context, $isoverride)
            ) {
                return false;
            }
            return true;
     ...
     * @return array
     */
    public static function get_requiresafeexambrowser_options(
        context_course|context_module $context,
        bool $isoverride = false,
    ): array {
        $options = [];

        if (self::can_donotrequire($context, $isoverride)) {
            $options[self::USE_SEB_NO] = get_string('no');
        }

        if (self::can_configure_manually($context, $isoverride)) {
            $options[self::USE_SEB_CONFIG_MANUALLY] = get_string('seb_use_manually', 'quizaccess_seb');
        }

        if (self::can_use_seb_template($context, $isoverride)) {
        ...
        if (self::can_upload_seb_file($context, $isoverride)) {
            $options[self::USE_SEB_UPLOAD_CONFIG] = get_string('seb_use_upload', 'quizaccess_seb');
        }

        if (self::can_use_seb_client_config($context, $isoverride)) {
     ...
     *
     * @param context_course|context_module $context Context to check access in.
     * @param bool $isoverride If true, check for override capability instead of normal capability.
     * @return bool
     */
    public static function can_use_seb_client_config(context_course|context_module $context, bool $isoverride = false): bool {
        return $isoverride
            ? has_capability('quizaccess/seb:manage_seb_usesebclientconfig_override', $context)
            : has_capability('quizaccess/seb:manage_seb_usesebclientconfig', $context);
    }
     ...
     * @return bool
     */
    public static function can_use_seb_template(context_course|context_module $context, bool $isoverride = false): bool {
        return $isoverride
            ? has_capability('quizaccess/seb:manage_seb_templateid_override', $context)
            : has_capability('quizaccess/seb:manage_seb_templateid', $context);
    }
     ...
     * @return bool
     */
    public static function can_upload_seb_file(context_course|context_module $context, bool $isoverride = false): bool {
        return $isoverride
            ? has_capability('quizaccess/seb:manage_filemanager_sebconfigfile_override', $context)
            : has_capability('quizaccess/seb:manage_filemanager_sebconfigfile', $context);
    }
     ...
     * @return bool
     */
    public static function can_configure_manually(context_course|context_module $context, bool $isoverride = false): bool {
        if ($isoverride && !has_capability('quizaccess/seb:manage_seb_configuremanually_override', $context)) {
            return false;
        }
     ...
     * @return bool
     */
    public static function can_donotrequire(context_course|context_module $context, bool $isoverride = false): bool {
        return $isoverride
            ? has_capability('quizaccess/seb:manage_seb_donotrequiresafeexambrowser_override', $context)
            : has_capability('quizaccess/seb:manage_seb_donotrequiresafeexambrowser', $context);
    }
