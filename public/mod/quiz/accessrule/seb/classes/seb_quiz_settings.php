...
class seb_quiz_settings extends persistent {
     ...
     * @param ?int $overrideid Override id, if null, attempt to fetch the ID in the function.
     * @return null|\quizaccess_seb\seb_quiz_settings
     */
    public static function get_by_quiz_id(int $quizid, ?int $overrideid = null): ?self {
        $overrideid = self::process_overrideid($quizid, $overrideid);
     ...
     *
     * If the $overrideid is null, this function will fetch the override ID based on the global $USER->id. If no override is found,
     * returns 0.
     *
     * @param int $quizid Quiz id.
     * @param ?int $overrideid Override id.
     * @return int $overrideid The processed override id.
     */
    protected static function process_overrideid(int $quizid, ?int $overrideid = null): int {
        if (is_null($overrideid)) {
            global $DB, $USER;

            $table = self::TABLE;
            $params = ['quizid' => $quizid, 'userid' => $USER->id];
