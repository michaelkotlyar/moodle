...
class overrides_controller extends access_rule_overrides_controller_base {
    ...
    public function add_form_fields(edit_override_form $form): bool {
        ...
        if (settings_provider::can_override_donotrequire($this->context)) {
            $requireseboptions[settings_provider::USE_SEB_NO] = get_string('no');
        }

        if (
            settings_provider::can_configure_manually($this->context) ||
        ...
        if (
            settings_provider::can_use_seb_template($this->context) ||
            settings_provider::is_conflicting_permissions($this->context)
        ) {
    ...
    public function save_settings(stdClass $override): void {
        ...
        $seboverrideid = $DB->get_field(
            'quizaccess_seb_quizsettings',
            'id',
            ['quizid' => $override->quiz, 'overrideid' => $override->overrideid],
        );

        if ($seboverrideid) {
            $sebquizsettings = seb_quiz_settings::get_by_quiz_id($override->quiz, $override->id);
    ...
    public function delete_settings(array $overrides): void {
        ...
        $DB->delete_records_select('quizaccess_seb_quizsettings', "overrideid $insql", $inparams);

        foreach ($ids as $id) {
            if ($sebquizsettings = seb_quiz_settings::get_by_quiz_id($this->quiz->id, $id)) {
