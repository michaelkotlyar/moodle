...
class override_manager {
    ...
    protected function get_user_override(int $userid): ?stdClass {
        ...
        [$selects, $joins, $params] = $this->accessmanager->get_override_settings_sql();
        ...
        $userparams = [...$params, ...[$this->quiz->id, $userid]];
    ...
    /**
     * Fetches the user override, if user belongs to multiple group overrides, combine to return final override settings.
     *
     * If the user has a user targeted override, this will take precedence over any group overrides. Otherwise the function will
     * check if a user has group overrides. If multiple group overrides found, the function will combine the settings to form one
     * override setting object.
     *
     ...
     * @return ?stdClass $override The final override settings object. Returns null if no overrides found.
     */
    public function get_effective_override(int $userid): ?stdClass {
        // Get user override, if null create empty object.
        ...
        // Retrieve group objects, combine into one override object.
        $gpoverrides = $this->get_group_overrides($userid);

        if ($gpoverrides) {
            // If only one group override, return it.
            if (count($gpoverrides) === 1) {
                return reset($gpoverrides);
            }
