...
class access_manager {
    ...
    public function __construct(quiz_settings $quizobj, int $timenow, bool $canignoretimelimits) {
        ...
        $this->overriderules = $this->make_override_rules($quizobj);
    }
     ...
     * @param quiz_settings $quizobj The current quiz used to instantiate the access rules.
     * @return array a list of access rule override controllers
     */
    protected function make_override_rules(quiz_settings $quizobj): array {
        $rulelist = [];
        foreach (core_component::get_plugin_list_with_class('quizaccess', 'overrides_controller') as $rule) {
            $rulelist[] = new $rule($quizobj);
        }
        return $rulelist;
    }
     ...
     */
    public function add_override_table_fields(
        stdClass $override,
        array $fields,
        array $values,
    ): array {
     ...
     *
     * Called by the override manager when combining override settings.
     *
     * @see mod_quiz\local\override_manager::get_combined_override()
